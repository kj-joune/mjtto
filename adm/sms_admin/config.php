...
if (!$config['cf_icode_server_ip'])   $config['cf_icode_server_ip'] = 'mail.ksvc.co.kr';
...
<div class="local_desc01 local_desc">
    <p>
        SMS 기능을 사용하시려면 먼저 시스템에 서비스 신청을 하셔야 합니다.<br>
        <a href="https://syshost.co.kr/board.php?menu=07&pagenos=07-4&board=freeboard&command=write&config=1&tname=SMS<?php echo rawurlencode("서비스 신청"); ?>" target="_blank">SMS서비스 신청</a>
    </p>
</div>
<?php } ?>

<?php
if ($config['cf_sms_use'] == 'icode') { // 시스템 사용
?>
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr class="icode_old_version">
        <th scope="row"><label for="cf_icode_id">시스템 회원아이디<br>(구버전)<strong class="sound_only"> 필수</strong></label></th>
        <td>
            <?php echo help("시스템에서 사용하시는 회원아이디를 입력합니다."); ?>
            <input type="text" name="cf_icode_id" value="<?php echo $config['cf_icode_id']; ?>" id="cf_icode_id" class="frm_input">
        </td>
    </tr>
    <tr class="icode_old_version">
        <th scope="row"><label for="cf_icode_pw">시스템 비밀번호<br>(구버전)<strong class="sound_only"> 필수</strong></label></th>
        <td>
            <?php echo help("시스템에서 사용하시는 비밀번호를 입력합니다."); ?>
        ...
        <td>
            <?php
                if ($userinfo['payment'] == 'A') {
                   echo '충전제';
                    echo '<input type="hidden" name="cf_icode_server_port" value="">';
    ...
    <tr class="icode_old_version">
        ...
        <td>
            ...
            <a href="https://syshost.co.kr/board.php?menu=07&pagenos=07-4&board=freeboard&command=write&config=1&tname=<?php echo rawurlencode("충전요청"); ?>" target="_blank" class="btn_frmline">충전요청</a>
        </td>
    </tr>
    <?php } ?>
    <tr class="icode_json_version">
        <th scope="row"><label for="cf_icode_token_key">시스템 토큰키<br>(JSON버전)</label></th>
        <td>
            <?php echo help("시스템 JSON 버전의 경우 시스템 토큰키를 입력시 실행됩니다.<br>SMS 전송유형을 LMS로 설정시 90바이트 이내는 SMS, 90 ~ 2000 바이트는 LMS 그 이상은 절삭 되어 LMS로 발송됩니다."); ?>
            ...
            <?php echo help("시스템 사이트 -> 토큰키관리 메뉴에서 생성한 토큰키를 입력합니다."); ?>
            <br>
...
<section>
    ...
    <div class="local_desc01 local_desc">
        <p>
            ...
            SMS 사용 설정은 <a href="../config_form.php#anc_cf_sms" class="btn_frmline">환경설정 &gt; 기본환경설정 &gt; SMS설정</a> 에서 SMS 사용을 시스템로 변경해 주셔야 사용하실수 있습니다.
        </p>
    </div>
</section>
