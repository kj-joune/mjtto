...
*/
// 시스템 사용자정보 (SYSK userinfo.php 연동)
function get_icode_userinfo($id, $pass)
{
    global $config;
    ...
    if ($host === '' || $user === '' || $domain === '' || trim((string)$pass) === '') {
        return $userinfo;
    }

    $url = 'https://' . $host . '/SMS/api/userinfo.php'
         . '?user='   . urlencode($user)
    ...
    //error_log('ICODE host='.$host);
    //error_log('ICODE url='.$url);
    //error_log('ICODE raw='.$res);

    if ($res === '') {
        error_log('ICODE ret='.print_r($userinfo, true));
        return $userinfo;
    }

    $parts = explode(';', $res);

    $code = isset($parts[0]) ? trim((string)$parts[0]) : '';
    $coin = isset($parts[1]) ? (int)trim((string)$parts[1]) : 0;
    $gpay = isset($parts[2]) ? (int)trim((string)$parts[2]) : 0;
    $pay  = isset($parts[3]) ? trim((string)$parts[3]) : 'A';

    if (strtoupper($code) === 'OK') {
        $userinfo['code']    = 0;
        $userinfo['coin']    = $coin;
        $userinfo['gpay']    = $gpay;
        $userinfo['payment'] = ($pay === 'C') ? 'C' : 'A';
    }
...
function is_sms_send($is_type=''){
    ...
    if($config['cf_icode_id'] && $config['cf_icode_pw']) {
        ...
        if($userinfo['code'] == 0 || $userinfo['code'] == '0') {
