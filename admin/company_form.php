...
function mjtto_company_query($sql) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($company_name === '') alert('제휴사명을 입력하세요.');
    ...
    if (!$is_update) {
        ...
        $new_company_code = mjtto_next_contract_code();
        if ($new_company_code === '') alert('제휴사코드 자동생성 범위를 초과했습니다.');
        $new_company_code_sql = sql_real_escape_string($new_company_code);

        sql_query("START TRANSACTION", false);

        mjtto_company_query("\n            INSERT INTO mz_company\n            SET\n                parent_company_id = '0',\n                company_name      = '{$company_name_sql}',\n                company_code      = '{$new_company_code_sql}',\n                company_type      = 'CONTRACT',\n                coupon_prefix     = NULL,\n                status            = '{$status_sql}'\n        ");

        $new_company_id = sql_insert_id();
        if (!$new_company_id) {
            sql_query("ROLLBACK", false);
            alert('제휴사 생성에 실패했습니다.');
        }
        ...
        if (!$admin_status_sql) {
            mjtto_company_query(" UPDATE g5_member SET mb_intercept_date = '".date('Ymd')."' WHERE mb_id = '{$admin_mb_id_sql}' ");
        }

        mjtto_company_query("\n            INSERT INTO mz_company_user\n            SET\n                company_id = '{$new_company_id}',\n                mb_id      = '{$admin_mb_id_sql}',\n                role_code  = 'COMPANY_ADMIN',\n                status     = '1'\n        ");

        sql_query("COMMIT", false);
        alert('제휴사와 관리자 계정이 등록되었습니다.', './company_list.php');
    } else {
        sql_query("START TRANSACTION", false);
        ...
        if ($admin_pass !== '') {
            mjtto_company_query(" UPDATE g5_member SET mb_password = '".get_encrypt_string($admin_pass)."' WHERE mb_id = '{$admin_mb_id_sql}' ");
        }

        if ($admin_status_sql) {
            mjtto_company_query(" UPDATE g5_member SET mb_intercept_date = '', mb_leave_date = '' WHERE mb_id = '{$admin_mb_id_sql}' ");
        } else {
            mjtto_company_query(" UPDATE g5_member SET mb_intercept_date = '".date('Ymd')."' WHERE mb_id = '{$admin_mb_id_sql}' ");
        }

        mjtto_company_query(" UPDATE mz_company_user SET status = '1' WHERE company_id = '{$company_id}' AND mb_id = '{$admin_mb_id_sql}' AND role_code = 'COMPANY_ADMIN' ");

        sql_query("COMMIT", false);
        alert('제휴사 정보가 수정되었습니다.', './company_list.php');
    }
}

$g5['title'] = $is_update ? '제휴사 수정' : '제휴사 등록';
...
<div class="box">
    ...
    <form method="post" action="" autocomplete="off">
        ...
        <table class="table-form">
            <tr>
                <th>제휴사명</th>
                <td><input type="text" name="company_name" value="<?php echo get_text($write['company_name']); ?>" class="input" maxlength="100" required></td>
            </tr>
            <tr>
                <th>제휴사코드</th>
                <td><span class="readonly-box"><?php echo $is_update ? get_text($write['company_code']) : '저장 시 자동 생성'; ?></span></td>
            </tr>
            <tr>
                <th>제휴사 상태</th>
                <td class="radio-wrap">
        ...
        <table class="table-form">
            ...
            <tr>
                <th>비밀번호</th>
                <td>
                    <input type="password" name="admin_pass" value="" class="input" maxlength="100" <?php echo $is_update ? '' : 'required'; ?>>
                    <div class="desc"><?php echo $is_update ? '비워두면 기존 비밀번호 유지' : '제휴사 관리자 로그인 비밀번호'; ?></div>
                </td>
            </tr>
            <tr>
                <th>비밀번호 확인</th>
                <td><input type="password" name="admin_pass2" value="" class="input" maxlength="100" <?php echo $is_update ? '' : 'required'; ?>></td>
            </tr>
            <tr>
                <th>관리자 이름</th>
                <td><input type="text" name="admin_name" value="<?php echo get_text($write['admin_name']); ?>" class="input" maxlength="100" required></td>
            </tr>
            <tr>
                <th>휴대폰</th>
                <td><input type="text" name="admin_hp" value="<?php echo get_text($write['admin_hp']); ?>" class="input" maxlength="20" required></td>
            </tr>
            <tr>
                <th>이메일</th>
                <td><input type="text" name="admin_email" value="<?php echo get_text($write['admin_email']); ?>" class="input" maxlength="100"></td>
            </tr>
            <tr>
                <th>계정 상태</th>
                <td class="radio-wrap">
        ...
        <div class="btns">
            ...
            <a href="./company_list.php" class="btn-list">목록</a>
        </div>
    </form>
</div>
<script>
