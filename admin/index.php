...
<?php if ($auth['role'] === 'SUPER_ADMIN') { ?>
    <div class="card">
        <h2>제휴사 관리</h2>
        <p>제휴사 등록, 수정, 관리자 계정 연결을 처리합니다.</p>
		<div class="btn-row">
			<a href="./company_list.php" class="btn btn-primary">제휴사목록</a>
			<a href="./company_form.php" class="btn btn-light">제휴사등록</a>
		</div>
    </div>
    <div class="card">
        <h2>전체 발권 현황</h2>
        <p>전체 제휴사·지점 발권 내역과 상태를 조회합니다.</p>
		<div class="btn-row">
			<a href="./issue_list.php" class="btn btn-primary">발권리스트</a>
		</div>
    </div>
    <div class="card">
        <h2>전체 경품 지급</h2>
        <p>모든 회차의 지급 요청, 승인, 완료 상태를 통합 관리합니다.</p>
		<div class="btn-row">
			<a href="./claim_list.php" class="btn btn-primary">지급목록</a>
			<a href="./prize_form.php" class="btn btn-light">기본 경품설정</a>
 		</div>
   </div>
    <div class="card">
        <h2>제휴사별 경품 설정</h2>
        <p>제휴사별 3~5등 경품을 바로 열어 등록·수정합니다.</p>
        <form method="get" action="./prize_form.php" class="inline-form">
            <select name="company_id" required>
                <option value="">제휴사를 선택하세요</option>
                <?php foreach ($contract_prize_options as $contract_row) { ?>
                <option value="<?php echo (int)$contract_row['company_id']; ?>"><?php echo get_text($contract_row['company_name']); ?></option>
                <?php } ?>
            </select>
            <button type="submit" class="btn btn-light">경품설정 열기</button>
        </form>
    </div>
    <div class="card">
        <h2>월별 정산</h2>
        <p>월 기준 발권 현황과 지급 요청 현황을 정리합니다.</p>
		<div class="btn-row">
			<a href="./settlement_month.php" class="btn btn-primary">월정산 보기</a>
 		</div>
   </div>
    <div class="card">
        <h2>운영 보조</h2>
        <p>추첨 데이터 동기화 등 운영 보조 기능입니다.</p>
 		<div class="btn-row">
		   <a href="./lotto_sync.php" class="btn btn-light">동행복권 동기화</a>
		</div>
    </div>
<?php } ?>
...
<?php if ($auth['role'] === 'COMPANY_ADMIN') { ?>
    <div class="card">
        <h2>지점 관리</h2>
        <p>소속 지점 등록, 수정, 관리자 연결을 처리합니다.</p>
		<div class="btn-row">
			<a href="./branch_list.php" class="btn btn-primary">지점 목록</a>
			<a href="./branch_form.php" class="btn btn-light">지점 등록</a>
 		</div>
   </div>
    <div class="card">
        <h2>소속 발권 현황</h2>
        <p>소속 지점 전체 발권 현황과 당첨 티켓을 추적합니다.</p>
		<div class="btn-row">
			<a href="./issue_list.php" class="btn btn-primary">발권리스트</a>
		</div>
    </div>
    <div class="card">
        <h2>제휴사 경품 지급</h2>
        <p>3~5등 지급 요청을 승인·보류·완료 처리합니다.</p>
		<div class="btn-row">
	        <a href="./claim_list.php" class="btn btn-primary">지급목록</a>
            <a href="./prize_form.php" class="btn btn-light">경품설정</a>
 		</div>
   </div>
    <div class="card">
        <h2>월별 정산</h2>
        <p>소속 지점 발권과 지급 요청 현황을 월 기준으로 확인합니다.</p>
		<div class="btn-row">
	        <a href="./settlement_month.php" class="btn btn-primary">월정산 보기</a>
		</div>
    </div>
...
<?php if ($auth['role'] === 'BRANCH_ADMIN' || $auth['role'] === 'ISSUER') { ?>
    <div class="card">
        <h2>발권 관리</h2>
        <p>지점 기준 발권 실행, 발권 내역 조회, 당첨 티켓 확인.</p>
		<div class="btn-row">
			<?php if ($auth['role'] !== 'ISSUER') { ?><a href="./issue_form.php" class="btn btn-primary">발권하기</a><?php } ?>
    ...
    <div class="card">
        <h2>지급 요청 / 확인</h2>
        <p>당첨 티켓의 지급 요청 등록과 현재 처리 상태를 확인합니다.</p>
		<div class="btn-row">
	        <a href="./claim_list.php" class="btn btn-primary">지급목록</a>
		</div>
    </div>
    <div class="card">
        <h2>월별 정산</h2>
        <p>내 지점의 월별 발권 현황과 지급 요청 현황을 확인합니다.</p>
		<div class="btn-row">
	        <a href="./settlement_month.php" class="btn btn-primary">월정산 보기</a>
		</div>
    </div>
